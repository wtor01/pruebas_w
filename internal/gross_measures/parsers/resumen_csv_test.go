...
import (
	...
	"bitbucket.org/sercide/data-ingestion/internal/gross_measures/parsers/fixtures/csv_resumen"
...
func Test_Unit_Services_ResumenCSV_IsFile(t *testing.T) {
	tests := map[string]struct {
		...
		"should be false if invalid path": {
			input: gross_measures.HandleFileDTO{
				FilePath: "XXX/0130_ZIV0004480340_S05_20220325111546W_220325111852",
			},
			want: false,
		},
		"should be false if invalid path 2": {
			input: gross_measures.HandleFileDTO{
				FilePath: "bucketname/distributorID/0130_ZIV0004480340_S05_20220325111546W_220325111852",
			},
			want: false,
		},
		...
		"should be false if invalid format filename": {
			input: gross_measures.HandleFileDTO{
				FilePath: "bucketname/DistribuidorX/Input/CSV/0130_ZIV0004480340_20220325111546W_220325111852.csv",
			},
			want: false,
		},
...
func Test_Unit_Services_ResumenCSV_Handle(t *testing.T) {
	tests := map[string]struct {
		input             gross_measures.HandleFileDTO
		...
		"should return error if error in create storage": {
			input: gross_measures.HandleFileDTO{
				FilePath: "bucketname/DistribuidorX/Input/Prime/0130_ZIV0004480340_S05_20220325111546W_220325111852",
			},
			readAllErr:        nil,
			storageCreatorErr: errors.New(""),
			want:              []gross_measures.MeasureCloseWrite{},
			err:               errors.New(""),
			fixtureFile:       "fixtures/S05/0130_ZIV0004480340_S05_20220325111546W_220325111852",
		},
		...
		"should parse 0130_ZIV0004480340_S05_20220325111546W_220325111852.csv file well": {
			input: gross_measures.HandleFileDTO{
				FilePath: "bucketname/DistribuidorX/Input/CSV/0130_ZIV0004480340_S05_20220325111546W_220325111852.csv",
			},
			readAllErr:        nil,
			storageCreatorErr: nil,
			want:              csv_resumen.Result_0130_ZIV0004480340_S05_20220325111546W_220325111852,
			err:               nil,
			fixtureFile:       "fixtures/csv_resumen/0130_ZIV0004480340_S05_20220325111546W_220325111852.csv",
		},
	}
	loc, _ := time.LoadLocation("Europe/Madrid")
	for testName, _ := range tests {
